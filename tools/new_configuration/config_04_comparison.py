...
def get_section(user_id: str, admin: bool = False) -> None:
    ...
    with ui.element("div").classes("w-full flex justify-end"):
        ...
        with Store(
                ...
                default_llm, label="LLM interface for comparison", options=llm_names) as llm_select:
        ...
        if admin:
            with Store(
                    ui.checkbox, AccessModel.set_comparison_llm,
                    AccessModel.get_comparison_llm(), text="User access") as checkbox:
                pass

        default_data = data_names[0] if 0 < len(data_names) else None
        with Store(
                ui.select, lambda name: ConfigModel.set_retrieval_data(user_id, name),
                default_data, label="Data interface for comparison", options=data_names) as data_select:
        ...
        if admin:
            with Store(
                    ui.checkbox, AccessModel.set_comparison_data,
                    AccessModel.get_comparison_data(), text="User access") as checkbox:
                pass
